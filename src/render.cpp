...
//#define DUMP_BUTTONS
  ...
  if (needs_remaster) {
    LPDIRECT3DTEXTURE9 pTemp = nullptr;
    ...
    hr = D3DXCreateTextureFromFileInMemoryEx_Original (
           pDevice, pSrcData, SrcDataSize, Width, Height,
             1, Usage, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM,
               Filter, 0, ColorKey, pSrcInfo, pPalette,
                 &pTemp );
    ...
    if (SUCCEEDED (hr))
    {
      dll_log.Log ( L" Loaded Texture in %4.02f ms (%4.02f MTex/s)",
                    (double)(time1.QuadPart - time0.QuadPart) /
                   ((double)freq.QuadPart / 1000.0),
                 ((Width * Height) * 1.33333) / ((double)(time1.QuadPart - time0.QuadPart) / (double)(freq.QuadPart)) / 1000000.0 );

      //dll_log.Log (L"Created Temporary System Memory Texture");

      D3DSURFACE_DESC desc;
      pTemp->GetLevelDesc (0, &desc);

      hr = D3DXCreateTexture (pDevice, desc.Width, desc.Height, D3DX_DEFAULT,
                              Usage | D3DUSAGE_AUTOGENMIPMAP, desc.Format,
                              D3DPOOL_DEFAULT, ppTexture);

      if (SUCCEEDED (hr))
      {
        //dll_log.Log (L"Created Autogen Texture");

        pTemp->AddDirtyRect        (nullptr);

        // Blit the source texture into the autogen texture
        hr = pDevice->UpdateTexture (pTemp, *ppTexture);

        if (SUCCEEDED (hr)) {
          //dll_log.Log (L"Updated Autogen Texture");
          //(*ppTexture)->GenerateMipSubLevels ();
        }
        else {
          dll_log.Log (L"Failed to Update Autogen Texture");
          (*ppTexture)->Release ();
        }
      }

      // Destroy the original incorrectly mipmapped texture
      pTemp->Release ();
    }

    if (FAILED (hr))
      dll_log.Log (L" Mipmap Autogen Failed! (%s)",
                   _com_error (hr).ErrorMessage () );
#endif
  }

  // We either do not need remastering, or we failed to autogen mipmaps.
  if ((! needs_remaster) || FAILED (hr)) {
   hr = D3DXCreateTextureFromFileInMemoryEx_Original (
      pDevice, pSrcData, SrcDataSize, Width, Height,
